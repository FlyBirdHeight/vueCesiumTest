...
<div>
    <el-form ref="polyline" size="mini" :model="polyline" :label-position="'top'">
        <el-form-item label="宽度">
            <el-input-number v-model="polyline.width" :precision="1" :step="10"></el-input-number>
        </el-form-item>
        ...
        <el-form-item label="是否启用虚线" v-if="polyline.outline == false">
        ...
        <el-form-item label="地形及模型附着">
            <el-select v-model="polyline.classificationType" placeholder="请选择">
                <el-option v-for="classif in classificationTypeSelect" :key="classif.value" :label="classif.label" :value="classif.value">
                </el-option>
            </el-select>
        </el-form-item>
        <el-form-item label="是否显示轮廓" v-if="polyline.dash == false">
        ...
        <el-form-item label="动态绘制">
            <el-button icon="el-icon-circle-plus" type="success" size="mini" @click="drawPolyline">点击开始动态绘制</el-button>
